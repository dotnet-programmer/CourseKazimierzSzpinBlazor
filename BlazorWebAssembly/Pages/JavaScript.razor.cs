...
public partial class JavaScript
{
	...
	private async Task DisplayResultJS() => await JSRuntime.InvokeVoidAsync("addNumberJS", 1, 2);
	...
	private async Task DisplayResultCSharp() => await JSRuntime.InvokeVoidAsync("addNumberCSharp", 2, 4);
	[JSInvokable]
	public static string GetCurrentDate() => DateTime.Now.ToString("d");
	private string _currentDate = string.Empty;
	private async Task DisplayDateCSharp() => _currentDate = await JSRuntime.InvokeAsync<string>("GetCurrentDateCSharp");

	// moduł JS
	private IJSObjectReference _jsModule;
	protected override async Task OnInitializedAsync() => _jsModule = await JSRuntime.InvokeAsync<IJSObjectReference>("import", "./scripts/jsTestModule.js");
	private async Task DisplayResultJsModule() => await _jsModule.InvokeVoidAsync("addNumberJSModule", 5, 5);

	// używanie bibliotek JS w C#
	[Inject]
	public IToastrService ToastrService { get; set; }
	private async Task DisplayToastrNotification() => await ToastrService.ShowInfoMessage("Toastr wywołany w Blazor!");
	...
	private async Task ChangeBackgroundColor() => await JSRuntime.InvokeVoidAsync("changeBackgroundColor");
}